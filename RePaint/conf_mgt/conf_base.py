...
class Default_Conf(NoneDict):
    ...
    def get_dataloader(
        self, dset="train", dsName=None, batch_size=None, return_dataset=False
    ):
    ...
    def get_debug_variance_path(self):
        return os.path.expanduser(
            os.path.join(
                self.get_default_eval_conf()["paths"]["root"], "debug/debug_variance"
            )
        )
    ...
    def device():
        ...
        sys.path.append(
            os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "..", "ICT", "Transformer"
            )
        )
    ...
    def eval_imswrite(
        ...
        if lrs is not None:
            lrs_dir_path = expanduser(self["data"][dset][name]["paths"]["lrs"])
