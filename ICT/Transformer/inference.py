...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--BERT", action="store_true", help="BERT model, Image Completion"
    )
    ...
    parser.add_argument(
        "--save_url", type=str, default="./", help="save the output results"
    )
    ...
    parser.add_argument(
        "--sample_all",
        action="store_true",
        help="sample all pixel together, ablation use",
    )
    parser.add_argument(
        "--skip_number",
        type=int,
    ...
    if opts.BERT:
        ...
        for x_name, y_name in zip(img_list, mask_list):
            ...
            x = input_image.resize(
                (opts.image_size, opts.image_size), resample=Image.BILINEAR
            )
            x = torch.from_numpy(np.array(x)).view(-1, 3)
            x = x.float().cuda()
            a = (
                ((x[:, None, :] - C[None, :, :]) ** 2).sum(-1).argmin(1)
            )  # cluster assignments

            mask_url = os.path.join(opts.mask_url, y_name)
            input_mask = Image.open(mask_url).convert("L")
            y = input_mask.resize(
                (opts.image_size, opts.image_size), resample=Image.NEAREST
            )
            y = torch.from_numpy(np.array(y) / 255.0).view(-1).cuda()
            y = y > 0.5
            y = y.float()
            ...
            if opts.sample_all:
                pixels = sample_mask_all(
                    IGPT_model,
                    context=a_tensor,
                    length=opts.image_size * opts.image_size,
