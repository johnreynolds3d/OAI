...
def main_worker(gpu, opts):
    ...
    try:
        ...
        ## Define the dataset
        train_dataset = ImageNetDatasetMask(
            opts.data_path,
            C,
            mask_path=opts.mask_path,
            is_train=True,
            use_ImageFolder=opts.use_ImageFolder,
            image_size=opts.image_size,
            random_stroke=opts.random_stroke,
        )
        test_dataset = ImageNetDatasetMask(
            opts.validation_path,
            C,
            mask_path=opts.mask_path,
            is_train=False,
            use_ImageFolder=opts.use_ImageFolder,
        ...
        train_config = TrainerConfig(
            ...
            weight_decay=0,
            lr_decay=True,
            warmup_tokens=tokens_per_epoch / opts.world_size,
            final_tokens=train_epochs * tokens_per_epoch / opts.world_size,
        ...
        trainer = Trainer(
            IGPT_model, train_dataset, test_dataset, train_config, gpu, rank
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="/home/ziyuwan/workspace/data/",
        help="Indicate where is the training set",
    )
    parser.add_argument(
        "--mask_path", type=str, default="/home/ziyuwan/workspace/data/mask"
    )
    parser.add_argument(
        "--BERT", action="store_true", help="Use bert objective to train"
    )
    parser.add_argument(
        "--ImageNet", action="store_true", help="Training with ImageNet"
    )
    parser.add_argument(
        "--batch_size", type=int, default=2 * 6, help="16*8 maybe suitable for V100"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="While training, the freq of printing log",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="input sequence length = image_size*image_size",
    )
    ...
    parser.add_argument(
        "--GELU_2", action="store_true", help="use the new activation function"
    )

    parser.add_argument(
        "--random_stroke", action="store_true", help="use the generated mask"
    )
    ...
    parser.add_argument(
        "--dynamic_weight",
        action="store_true",
        help="Not mean directly, based on the mask regions",
    )

    parser.add_argument(
        "--use_ImageFolder",
        action="store_true",
        help="using the original folder for ImageNet dataset",
    )

    ### DDP+AMP
    parser.add_argument(
        "--DDP", action="store_true", help="using DDP rather than normal data parallel"
    )
    ...
    parser.add_argument(
        "--node_rank", type=int, default=0, help="the id of this machine"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="latest.pth",
        help="start from where, the default is latest",
    )
