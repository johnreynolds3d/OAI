...
class Trainer:
    ...
    def __init__(self, model, train_dataset, test_dataset, config, gpu, global_rank):
        ...
        self.train_sampler = DistributedSampler(
            train_dataset, num_replicas=config.world_size, rank=global_rank
        )
        self.test_sampler = DistributedSampler(
            test_dataset, num_replicas=config.world_size, rank=global_rank
        )
    ...
    def train(self, loaded_ckpt):
        ...
        if loaded_ckpt is not None:
            ...
            print(
                "Warning: There is no previous optimizer found. An initialized optimizer will be used."
            )
        ...
        for epoch in range(config.max_epochs):
            ...
            print(
                "Epoch: %d, test loss: %f, time for one epoch: %d seconds"
                % (epoch, test_loss, time.time() - epoch_start)
            )
            ...
            ):  ## Validation on the global_rank==0 process

                best_loss = test_loss
                print("current best epoch is %d" % (epoch))
                self.save_checkpoint(
                    epoch, optimizer, self.tokens, best_loss, save_name="best"
                )

            if not np.isnan(test_loss):
                self.save_checkpoint(
                    epoch, optimizer, self.tokens, best_loss, save_name="latest"
                )
            else:
