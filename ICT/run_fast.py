...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sample_num", type=int, default=1, help="# of completion results"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--Places2_Nature", action="store_true", help="Places2_Nature pretrained model"
    )
    parser.add_argument(
        "--ImageNet", action="store_true", help="ImageNet pretrained model"
    )
    parser.add_argument(
        "--visualize_all",
        action="store_true",
    ...
    if opts.ImageNet:
        # FAST VERSION: Use smaller image size (16x16 = 256 tokens instead of 32x32 = 1024)
        stage_1_command = (
            ...
            + prior_url
            + " \
                                --image_size 16 --n_samples "
            + str(opts.sample_num)
        )
    elif opts.FFHQ:
        # FAST VERSION: Use smaller image size (24x24 = 576 tokens instead of 48x48 = 2304)
        stage_1_command = (
