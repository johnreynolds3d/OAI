...
def generate_all_masks():
    ...
    for i, image_file in enumerate(image_files, 1):
        try:
            ...
            if i % 50 == 0 or i == len(image_files):
                print(f"Progress: {i}/{len(image_files)} ({i/len(image_files)*100:.1f}%) - {successful} successful, {failed} failed")
    ...
    mask_files = [f for f in os.listdir(mask_dir) if f.lower().endswith('.png') and f != 'create_mask.py']
...
def create_sample_visualization():
    ...
    cv2.rectangle(mask, (margin + 20, margin + 20), (margin + 20 + square_size, margin + 20 + square_size), 255, -1)
    
    # Create visualization
    # Show the safe zone boundary
    cv2.rectangle(sample_img, (margin, margin), (224 - margin, 224 - margin), (0, 255, 0), 2)
...
if __name__ == "__main__":
    ...
    print("This script is deprecated. Mask generation has been integrated into split.py")
