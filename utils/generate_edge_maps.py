...
def main():
    parser = argparse.ArgumentParser(
        description="Generate edge detection maps for ICT model"
    )
    parser.add_argument(
        "--input_dir", type=str, required=True, help="Input images directory"
    )
    parser.add_argument(
        "--output_dir", type=str, required=True, help="Output edge maps directory"
    )
    parser.add_argument("--sigma", type=float, default=2, help="Canny sigma parameter")
    parser.add_argument(
        "--threshold", type=float, default=0.5, help="Canny threshold parameter"
    )
