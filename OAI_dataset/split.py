...
def create_balanced_splits():
    ...
    osteo_subset = osteo_samples.sample(n=max_samples_per_class, random_state=42).reset_index(drop=True)
    normal_subset = normal_samples.sample(n=max_samples_per_class, random_state=42).reset_index(drop=True)
    ...
    print(f"Osteoporotic in balanced set: {len(balanced_df[balanced_df['is_osteo'] == True])}")
    print(f"Normal in balanced set: {len(balanced_df[balanced_df['is_osteo'] == False])}")
    ...
    output_dirs = ['train/img', 'train/mask', 'train/mask_inv', 'valid/img', 'valid/mask', 'valid/mask_inv', 'test/img', 'test/mask', 'test/mask_inv']
    ...
    # Generate masks for each split
    train_mask_success, train_mask_failed = generate_masks_for_split(train_df, "train", 'train/img', 'train/mask', 'train/mask_inv')
    val_mask_success, val_mask_failed = generate_masks_for_split(val_df, "validation", 'valid/img', 'valid/mask', 'valid/mask_inv')
    test_mask_success, test_mask_failed = generate_masks_for_split(test_df, "test", 'test/img', 'test/mask', 'test/mask_inv')
